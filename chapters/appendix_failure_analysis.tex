...
pipeline (CLIP-L, Instruction\,E, \texttt{+context} rung).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/qualitative_failure_q1100.png}
\caption{Failure mode (ii): domain-transfer with no database counterpart (``as a painting'').}
\end{figure}
\begin{figure}[htbp]
\centering
...
\begin{table}[htbp]
\centering
\caption{Bottom-30 queries by \emph{Ours} AP (Ours\,[\textsc{E}],
...
% ---------------------------------------------------------------------
\section*{Full List of Modification Texts}

For reference, and to give a sense of the full diversity behind the
token-level analysis above, Figure~\ref{fig:icir_text_queries} lists every
unique modification text in the i-CIR benchmark, in alphabetical order.

\begin{figure}[p]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.85\textheight,keepaspectratio]{figures/icir_text_queries}
  \caption{%
    The unique text queries of i-CIR, listed in alphabetical order. These
    queries reflect diverse compositional transformations across appearance,
    context, attributes, viewpoint, and more. Reproduced from
    Psomas~et~al.~\cite{psomas2025icir}.%
  }
  \label{fig:icir_text_queries}
\end{figure}
