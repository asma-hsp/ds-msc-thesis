...
\caption[Overview of the proposed retrieval pipeline.]{Overview of the
  ...
  centred, yielding $\bar{q}^v$, $\bar{q}^t$, and $\bar{c}$; each is compared
  against the precomputed embedding of every image in the database
  $\mathcal{X}$ via a dot product, producing three similarity scores
  $s^v_i$, $s^t_i$, $s^c_i$.
  \textit{Right:} the clamped triplet product
  $\tilde{s}^f_i = \mathrm{clamp}(s^v_i)\cdot\mathrm{clamp}(s^t_i)\cdot
  \mathrm{clamp}(s^c_i)$ acts as a soft logical \textsc{and} and ranks the
  database; the top-1 result (green) is the correct archival photograph of the
  same instance. Every component -- both encoders, the caption generator, and
  all fusion steps -- is off-the-shelf and frozen (zero-shot).\label{fig:pipeline}}
\end{figure}
