...
\begin{figure}[h!]
\centering
% HIGHLIGHT: create figures/pipeline_overview.png (or .pdf). Suggested content:
%   left: query image q^v and query text q^t; q^v + q^t -> MLLM -> caption c;
%   middle: three encoders -> three similarities s_v, s_t, s_c;
%   right: clamp-product fusion -> ranked database.
...
proposed pipeline: the reference image and modification text are encoded by a
frozen vision--language model; a frozen MLLM generates a target caption; the
image, text and caption similarities are fused by the clamped triplet product
and used to rank the database.\label{fig:pipeline}}
\end{figure}
