...
text alone, or an example image alone, often cannot express. A user who
remembers \emph{a particular handbag, but in black instead of brown}, or
\emph{this building, photographed at night}, has a target in mind that is
naturally described as an existing image together with a desired change. This
...
training set~\cite{vo2019tirg}. However, instance-level annotation is
expensive, the i-CIR benchmark is designed for evaluation rather than
large-scale training, and supervised fusion models tend to overfit the
distribution of modifications seen during training. This motivates the
\emph{zero-shot} setting~\cite{saito2023pic2word,baldrati2023circo}, in which
no CIR-specific training is performed and retrieval is built entirely on
...
\begin{itemize}
  \item \textbf{An MLLM-generated target caption as a third retrieval modality.}
        ...
        target image with a frozen MLLM (InternVL-3.5-8B), and to use the
  ...
  \item \textbf{A backbone comparison on the full i-CIR benchmark.} We embed
        ...
        (CLIP ViT-L/14, CLIP ViT-H/14, SigLIP ViT-L/16, and SigLIP2 g/16-384)
  ...
  \item \textbf{An honest negative result.} We show that reranking the
        top-ranked candidates with self-supervised visual features
        (DINOv2/DINOv3) \emph{hurts} retrieval quality, and explain why.
\end{itemize}
...
zero-shot CIR, and the i-CIR benchmark.
Chapter~\ref{chapt:method} describes the proposed methodology: the problem
formulation, the evaluation protocol, the retrieval pipeline, the target-caption
generation step, and the triplet fusion.
