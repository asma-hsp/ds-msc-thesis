...
else: subtracting a precomputed mean vector $\mu$ and re-normalising,
...
than signal). Our own re-run of the released code tracks these numbers closely, but does not
match them exactly (the reproduced best rung reaches $32.48$ against the paper's
$34.35$). Two effects account for the residual gap. First, the contextualization
step draws fresh object names per query, so its contribution varies slightly
between runs. Second, and more importantly, we re-extracted all features in a
newer software stack, and the shortfall is concentrated in the \emph{image}
branch: the image-only score is lower here than in the original work, a
difference consistent with changes in image decoding and resizing across library
versions -- and possibly with the quality of the database images we were able to
re-download -- which lower the already-weak image signal. The text branch is
unaffected. Because our reproduction and the proposed method are evaluated in the
same environment, this gap does not affect the relative comparisons reported in
