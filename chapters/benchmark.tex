...
\end{itemize}
Figure~\ref{fig:icir_query} lays out this structure schematically, and
Figure~\ref{fig:icir_montage} shows it on a concrete example. A method that
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/icir_query_example}
  \caption{%
    A concrete i-CIR instance (the cartoon character \emph{Homer Simpson}).
    Each column is a different modification of the \emph{same} reference
    instance. The top row shows the \emph{composed positives} -- the correct
    targets. The middle row shows \emph{textual hard negatives}: a
    \emph{different} instance rendered under the same modification (e.g.\ a
    different action figure, a different plushie), which a text-only method
    would wrongly rank high. The bottom row shows \emph{composed hard
    negatives}: images that are on-topic but neither the right instance nor
    the right modification. Distinguishing the top row from the rows below it
    requires reading the image and the text together.%
  }
  \label{fig:icir_montage}
\end{figure}
...
``cartoon'', ``aerial view'', ``in a cloudy day''). From their centered text
embeddings a weighted \emph{contrastive covariance} matrix is formed,
...
the retrieval domain -- even generic ImageNet class names transfer well -- because
CLIP's stylistic directions are largely domain-independent. In practice only the
...
Instance retrieval often improves with \emph{query expansion}: after an initial
search, the query is nudged towards the images it already retrieved with high
...
was~\cite{philbin2007oxford}. The expanded query then replaces the original for
